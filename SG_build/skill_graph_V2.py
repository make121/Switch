...
class SkillGraphBuilder:
    ...
    def build_cross_skill_edges(self, graph: SkillGraph):
        ...
        Boundary filtering (Sec III-A.2 note): frames within the first/last
        `exclude_boundary_frames` of a skill are excluded from being either
        the src endpoint (skipped during sampling) or the dst endpoint
        (masked out before top-K nearest-neighbor selection). This prevents
        cross-skill edges from degenerating onto skill start/end frames,
        which tend to be kinematically generic and falsely "close" to many
        frames in other skills.
        """
        ...
        for src_skill in range(num_skills):
            ...
            for dst_skill in range(num_skills):
                ...
                sample_indices = [
                    t for t in range(0, len(src_feats), self.subsample_stride)
                    if not src_mask[t]
                ]
                ...
                for t_src in sample_indices:
                    ...
                    # Mask out target skill's boundary frames so they can
                    # never be selected as a nearest neighbor.
                    distances[dst_mask] = np.inf

                    # Top-K nearest (guard against topk >= array length,
                    # which np.argpartition cannot handle)
                    ...
                    for t_dst in top_indices:
                        ...
                        if not np.isfinite(d) or d > self.cross_skill_threshold:
                            continue
    ...
    def _boundary_mask(self, skill_len: int) -> np.ndarray:
        """True marks local frames within the start/end exclusion zone of a
        skill (first/last `exclude_boundary_frames` frames). These frames
        cannot be used as cross-skill edge endpoints (src or dst).
        """
        n = self.exclude_boundary_frames
        mask = np.zeros(skill_len, dtype=bool)
        if n > 0:
            mask[:min(n, skill_len)] = True
            mask[max(0, skill_len - n):] = True
        return mask
